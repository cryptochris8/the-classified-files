...
document.addEventListener('DOMContentLoaded', function() {
    // Check if running on iOS/Android app - use In-App Purchase system
    if (typeof platformDetection !== 'undefined' && platformDetection.isNativeApp()) {
        console.log('Native app detected - Using In-App Purchase system');

        // iOS payment system will be initialized separately by ios-payment-system.js
        // For now, create a simple wrapper that delegates to the IAP system
        window.paymentSystem = {
            isUnlocked: false,
            isNativeApp: true,

            shouldShowPurchasePrompt: (caseId) => {
                // Check if case is purchased via IAP
                if (window.iosPaymentSystem) {
                    return !window.iosPaymentSystem.hasPurchased(caseId);
                }
                return true;
            },
            ...
            createPurchaseButton: (caseKey, caseName) => {
                const button = document.createElement('button');
                button.className = 'purchase-button';
                button.dataset.caseKey = caseKey;
                button.innerHTML = `
                    <span class="purchase-icon">💳</span>
                    <span class="purchase-text">Unlock ${caseName} - $4.99</span>
                `;
                ...
                button.addEventListener('click', async () => {
                    if (window.iosPaymentSystem) {
                        await window.iosPaymentSystem.purchaseCase(caseKey);
                        // Reload game state after purchase
                        location.reload();
                    }
                });
                ...
                return `
                    <div class="purchase-prompt">
                        <div class="purchase-content">
                            <h3>🔒 Premium Case File</h3>
                            <p><strong>${caseTitle}</strong> is part of the full game.</p>
                            <p>Unlock this premium case file with branching storylines, evidence collection, and multiple endings.</p>
                            <div class="price-options">
                                <button class="purchase-button" onclick="window.iosPaymentSystem?.purchaseCase('${caseKey}').then(() => location.reload())">
                                    Purchase ${caseTitle} - $4.99
                                </button>
                            </div>
                            <small>Secure payment powered by Apple In-App Purchase</small>
                        </div>
                    </div>
                `;
            },
            ...
            showPurchaseModalIfRequested: () => {
                // Could implement modal for iOS too, but not necessary for v1
            }
        };
    ...
    // Web browser - use Stripe payment system
    console.log('Web browser detected - Using Stripe payment system');
